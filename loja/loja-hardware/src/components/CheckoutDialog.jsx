    ...
    <div className="checkout-overlay fixed inset-0 z-[80] overflow-y-auto p-4">
      <div role="dialog" aria-modal="true" aria-labelledby="checkout-title" aria-describedby="checkout-description" className="checkout-dialog mx-auto my-4 w-full max-w-5xl rounded-3xl p-5 shadow-2xl sm:my-6 sm:p-8">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onClose} className="close-checkout text-2xl" aria-label="Fechar checkout">×</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="checkout-layout mt-7">
          ...
          <aside className="checkout-summary">
            ...
            <button disabled={isSubmitting || isLookingUpCep} className="admin-primary w-full cursor-pointer rounded-xl py-3 font-semibold disabled:cursor-wait disabled:opacity-60">{isSubmitting ? 'Redirecionando para pagamento...' : 'Ir para pagamento seguro'}</button>
            <p className="checkout-security"><span aria-hidden="true">✓</span> Você será direcionado ao ambiente seguro de pagamento.</p>
          </aside>
        </form>
      </div>
    </div>
